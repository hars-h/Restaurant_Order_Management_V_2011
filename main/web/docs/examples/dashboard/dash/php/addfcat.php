...
<nav class="main-menu">
        ...
        <div class="col-sm-9 col-sm-offset-3 col-md-10 col-md-offset-2 main">
          <h1 class="page-header" style="margin-top: 60px">Add Category</h1>
...
if (!isset($_POST['submit'])) {
  ?>
   <form action="<?=$_SERVER['PHP_SELF']?>" method="post">
      <table>       <th> CAT DESC</th> <th><input type="text" name="tableno1" style="color:black;" /></th><br /></tr>

      
 <tr>
        <th></th><th> <input style="color: black;" type="submit" name="submit" value="Add Category" /></th>
</table>
    </form>
...
## query database
 ...
 $exists = 0;
    $result = $db->query("SELECT CAT_DESC from f_cat WHERE CAT_DESC = '{$tableno1}' LIMIT 1");
    if ($result->num_rows == 1) {
        $exists = 1;
        
    } 
 
    if ($exists == 1) {echo "<p>Category already exists!</p>";
    echo "<a href=\"../restconf.php\">Try again...</a>";
    }
    else {
    
        $sql = "INSERT  INTO f_cat(`CAT_DESC`) 
                VALUES ('{$tableno1}')";
 
        if ($db->query($sql)) {
            //echo "New Record has id ".$mysqli->insert_id;
            echo "<p>Registred successfully!</p>";
              echo "<a href=\"../restconf.php\">Add New Category...</a>";
        } else {
           echo "not registered";
            exit();
   }
